...
<table><tr>
	...
	<td><ul>
		...
		<li><div class="item_title">Select the period of time:</div>
			<div class="item_content">
				<button id=day onclick="go_chart(acIp, ip, ssid,'day','%H:%M')">Last Day</button>
				<button id=week onclick="go_chart(acIp, ip, ssid,'week','%m/%d')">Last Week</button>
				<button id=month onclick="go_chart(acIp, ip, ssid,'month','%m/%d')">Last Month</button>
			</div>
		</li>
...
function monthly (){
	$( "#datepicker" ).datepicker({
		...
		onSelect: function(dateText, inst) {
			go_chart(acIp, ip, ssid,'monthly','%H:%M',dateText);
		}});
};
$(function () {
	   $("#choose_date").click(function() {
		   //if($("#from").val() && $("#to").val()!=""){
		   //go_chart(ip, ssid,'choose_date');
		   //}
		   if($("#from").val()==""){
			   ...
			   go_chart(acIp, ip, ssid,'choose_date','%m/%d');
...
var acIp, ip, ssid;
function go_chart(acIp, ip, ssid, time_select,time_format,dateText) {
	this.acIp = acIp;//get acIp value
	this.ip = ip;//get ip value
	this.ssid = ssid;//get ssid value
	//alert(dateText);
	$("#chart2").block({ 
...
function init_chart(acIp, ip, ssid, time_select, time_format, dateText) {
	var dataSet = {
			acIp: acIp,
			ip: ip,
            ssid: ssid,
            action: 'apSsidList_calendar'
	};
	$.ajax({
        type: "POST",
        url: "HistoryApSsidListChart",
	...
	go_chart(acIp, ip, ssid, time_select, time_format, dateText);
}

</script>
<!-- <button id="CallChart">Call Chart data</button> -->
<c:out value="<script>$(document).ready(init_chart('${param.acip}', '${param.apip}', '${param.ssid}','week','%m/%d'));</script>" escapeXml="false"></c:out>
</body>
</html>