...
<title><c:out value="${param.level == 'error' ? 'Alarm Viewer' : 'Log Viewer'}"/></title>
<script type="text/javascript" src="javascript/jquery-1.10.2.js"></script>
<link rel="stylesheet" type="text/css" href="theme/Theme1/main.css">
<link rel="stylesheet" type="text/css" href="theme/Theme1/log_viewer.css">
...
<%-- The Beginning of Page Content --%>
	...
	<jsp:include page="/LogViewer"/>
	
	<div id="page_content">
		<div id="page_title"><c:out value="${param.level == 'error' ? 'Alarm Viewer' : 'Log Viewer'}"/></div><br>
		
        <table id="time-selector-table">
            <tr><th class="time-selectotr-header">Select one single day:</th><th class="time-selectotr-header">Select the time range:</th></tr>
            <tr>
                <td class="time-selector-calendar"><div id="datepicker"></div></td>
                <td class="time-selector-fromto">
                    <table>
                        <tr><td><label for="from">From:</label></td><td><input type="text" id="log_from" name="from"></td></tr>
                        <tr><td><label for="to">To:</label></td><td><input type="text" id="log_to" name="to"></td><td><button id="log_fromto_apply">Apply</button></td></tr>
                    </table>
                </td>
            </tr>
        </table>
		...
		<tbody>
			<c:if test="${not empty logTable}">
				<c:forEach var="item" items="${logTable.logItems}">
					<tr>
						<td class="itemTime">${item.time}</td><td class="itemLevel">${item.level}</td><td class="itemMessage">${item.message}</td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
...
function init_log_calendar(first_log_time, last_log_time, selected_log_date) {
    if (!selected_log_date) selected_log_date = new Date();
	$( "#datepicker" ).datepicker({
		...
		onSelect: function(dateText, inst) {
		    window.location.href = "log_viewer.jsp?level=" + log_level + "&date=" + dateText;
		}}).datepicker('setDate', selected_log_date);
};
$(function () {
	   $("#log_fromto_apply").click(function() {
		   if($("#log_from").val()==""){
		       ...
		       window.location.href = "log_viewer.jsp?level=" + log_level + "&datefrom=" + $("#log_from").val() + "&dateto=" + $("#log_to").val();
		   }
	   });
});
function init_log_fromto(first_log_time, last_log_time, selected_log_from, selected_log_to) {
	$( "#log_from" ).datepicker({
		minDate: new Date(first_log_time),
		maxDate: new Date(last_log_time),
		dateFormat: 'yy-mm-dd',
...
function init_time_selector(first_log_time, last_log_time, selected_log_date, selected_log_from, selected_log_to, log_level) {
    this.log_level = (!log_level) ? "info" : log_level;
    init_log_calendar(first_log_time, last_log_time, selected_log_date);
    init_log_fromto(first_log_time, last_log_time, selected_log_from, selected_log_to);
}
...
<c:out value="<script>$(document).ready(init_time_selector('${firstLogTime}', '${lastLogTime}', '${param.date}', '${param.datefrom}', '${param.dateto}', '${param.level}'));</script>" escapeXml="false"></c:out>
</body>
</html>