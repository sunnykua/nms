...
<body>
      ...
      <div id="extra_${status.count}">
         <table id="portSection" class="interfaceTable_l">
				<tr VALIGN=TOP>
					<td><input type="hidden" id="switchid_${status.count}" value="${dev.publicIp}">
						...
						<tr>
						<c:if test="${dev.deviceType == 'l2switch' || dev.deviceType == 'l3switch' || dev.deviceType == 'firewall'}"><th>Port</th><th>Alias</th></c:if>
						</tr>
							...
							<c:if test="${inf.ifType == 'eth'}">
								<tr>
									...
									<td><input type="text" name="port_alias" style="width=200px;" value="${inf.aliasName}"></td>
									</c:if>
								</tr>
							</c:if>
							</c:forEach>
		...
		<div class="buttonSection"><button class="btn btn-primary btn-xs" id="portAliasApply_${status.count}">Apply</button></div>
		</div>
		</c:if>
		</c:forEach>
