...
<link rel="stylesheet" type="text/css" href="theme/Theme1/mail_queue.css">
...
<%@ include file="theme/Theme1/prefix.jspf"%>
	<jsp:include page="/QueueViewer">
		<jsp:param value="mail" name="type"/>
		<jsp:param value="read" name="action"/>
	</jsp:include>
	
	<div id="page_content">
		<div id="page_title">Mail Queue</div>
		...
		<c:forEach var="mailInfo" items="${mailQueueList}" varStatus="status">
			<table class="tableMailQueue" id="mailTable">
				<tr>
					<td rowspan="7" class="tdCount">${status.count}</td>
					<td rowspan="7" class="tdSelectCheck">
						<input type="checkbox" id="selectedCheck_${status.count}" name="selectItem" value="${mailInfo.id}" <c:if test="${sessionScope.userLevel>1}">disabled</c:if>>
					</td>
					<th class="thItemName">Time</th><td class="tdItemValue">${mailInfo.time}</td>
				</tr>
				<tr>
					<th class="thItemName">From</th><td class="tdItemValue"><c:out value="${mailInfo.from}" escapeXml="true"></c:out></td>
				</tr>
				<tr>
					<th class="thItemName">To</th><td class="tdItemValue"><c:out value="${mailInfo.to}" escapeXml="true"></c:out></td>
				</tr>
				<tr>
					<th class="thItemName">SMTP_Host</th><td class="tdItemValue">${mailInfo.smtpHost}</td>
				</tr>
				<tr>
					<th class="thItemName">Subject</th><td class="tdItemValue">${mailInfo.subject}</td>
				</tr>
				<tr>
					<th class="thItemName">Text</th><td class="tdItemValue">${mailInfo.text}</td>
				</tr>
				<tr>
					<th class="thItemName">Reason</th><td class="tdItemValue">${mailInfo.reason}</td>
				</tr>
			</table>
		</c:forEach>
	</div>
...
$(function () {
	$("#deleteBtn").click(function() {
		...
		if (selectedItems.length < 1) {
			alert("Please select at least one of the mails.");
			return;
		}
		
		if (confirm("Are you sure to delete these " + selectedItems.length + " mails?")) {
			$.ajax({
				type: "POST",
				url: "QueueViewer?type=mail&action=delete",
