...
<div id="area" style="width: 1024px;margin: auto;">
	<c:if test="${param.timeSelect == 'daily'}">
		<jsp:include page="/DeviceReport">
			<jsp:param value="periodicalReport" name="action"/>
			<jsp:param value="daily" name="timeSelect"/>
		</jsp:include>
	</c:if>
	<c:if test="${param.timeSelect == 'weekly'}">
		<jsp:include page="/DeviceReport">
			<jsp:param value="periodicalReport" name="action"/>
			<jsp:param value="weekly" name="timeSelect"/>
		</jsp:include>
	</c:if>
	<c:if test="${param.timeSelect == 'monthly'}">
		<jsp:include page="/DeviceReport">
			<jsp:param value="periodicalReport" name="action"/>
			<jsp:param value="monthly" name="timeSelect"/>
		</jsp:include>
	</c:if>
	
	<div style="font-size: 36px;font-weight: bold;text-align: center;font-style: italic;">Cybert Expert</div>
	<div style="font-size: 20px;font-weight: bold;text-align: center;background-color:#00bfff;">
		<font color="#ffffff">
			<c:if test="${param.timeSelect == 'daily'}">Daily</c:if>
			<c:if test="${param.timeSelect == 'weekly'}">Weekly</c:if>
			<c:if test="${param.timeSelect == 'monthly'}">Monthly</c:if> Report
		</font>
	</div>
