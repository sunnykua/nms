...
<title>Server Overview ${param.ip}</title>
...
function open_htchart1(/* link,  */ip) {
	...
	window.open(target,"htchart1","menubar=1,resizable=1,width=950,height=460");
}
...
function updateActiveConnection() {
    $.ajax({
        type: "POST",
        url: "SystemInfo?chart=get_active_connection",
        success: function(data) {
            if(data == "SignOut") {
                alert("Is Logout");
				location.href="logout.jsp";
			}
            $("#activeConnection").html(data);
            setTimeout(updateActiveConnection, 20000);
        }
    });
}
</script>
</head>
<body onload="createServerMenu('${param.ip}')">
...
<c:if test="${dev.publicIp==param.ip}">
    ...
    <table class="overViewTable_l">
        <caption>Server Information</caption>
	        <tr>
	            <td>Active Connections:</td><td><div id="activeConnection"></div></td>
	        </tr>
    </table>
    <c:out value="<script>$(document).ready(function() {update('${param.ip}'); updateActiveConnection();});</script>" escapeXml="false"></c:out>
    <hr>
	<table class="overViewTable_l">
			<tr>
				<td>
