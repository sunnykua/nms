...
function GetData() {
	portselect = [acip, apip, apssidindex, apssidname];
	var param = {
			acip: acip,
			apip: apip,
			apssidindex: apssidindex,
			apssidname: apssidname,
			chart: 'apssidlist_rx_tx_octet_bps',
			action: 'read',
...
function go_chart(acip, apip, apssidindex, apssidname, updateInterval) {
	this.acip = acip;
	this.apip = apip;
	this.apssidindex = apssidindex;
	this.apssidname = apssidname;
...
<div style="font-size: 1em; font-weight: bold;" >SSID:${param.apssidname} Data Rate (kbps) [acip:${param.acip}, apip:${param.apip}]</div>
<br>
...
<c:out value="<script>$(document).ready(go_chart('${param.acip}', '${param.apip}', '${param.apssidindex}', '${param.apssidname}', '${timeout}'));</script>" escapeXml="false"></c:out>
