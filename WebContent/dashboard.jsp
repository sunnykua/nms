...
<script type="text/javascript">
	...
	function getLocation() {
		...
		$.ajax({
			type : "POST",
			url : "Dashboard?method=getLocation",
	...
	function getSysInfo() {
		...
		$.ajax({
			type : "POST",
			url : "Dashboard?method=getSysInfo",
			success : function(data) {
	...
	function getDevices() {
		...
		$.ajax({
			type : "POST",
			url : "Dashboard?method=getDevices",
			success : function(data) {
	...
	function getErrorLog() {
		...
		$.ajax({
			type : "POST",
			dataType : "JSON",
			url : "Dashboard?method=getErrorLog",
			success : function(data) {
				//console.log(data);
	...
	var json = [
	    		{'addr': "110台灣台北市信義區忠孝東路五段51號", 'text': '<strong>110台灣台北市信義區忠孝東路五段51號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路五段151號", 'text': '<strong>110台灣台北市信義區忠孝東路五段151號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路五段232號", 'text': '<strong>110台灣台北市信義區忠孝東路五段232號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路五段344號", 'text': '<strong>110台灣台北市信義區忠孝東路五段344號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路五段444號", 'text': '<strong>110台灣台北市信義區忠孝東路五段444號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路五段544號", 'text': '<strong>110台灣台北市信義區忠孝東路五段544號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路五段644號", 'text': '<strong>110台灣台北市信義區忠孝東路五段644號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路五段44號", 'text': '<strong>110台灣台北市信義區忠孝東路五段44號</strong>'},
	            {'addr': "110台灣台北市信義區忠孝東路四段151號", 'text': '<strong>110台灣台北市信義區忠孝東路四段151號</strong>'}
	]
	
	function getAllLocation(LocalAddress) {
		$("#getLocationTime").html("Processing, please wait ...");
		//console.log(LocalAddress.toString());
		$.ajax({
			type : "POST",
			url : "Dashboard?method=getLocation",
			success : function(data) {				
				...
				if (data[1] === undefined || data[1] == "") {
					...
					$('#Location').append('<div id="ExternalLink" align="right"><a target="_blank" href="http://59.120.5.184/nms/google_maps.jsp?address='+LocalAddress+'" style="text-decoration: none"><font color="#0000FF"><b>External link</b></font></a>&nbsp;&nbsp;</div>');
				}
			}
		});
	}
...
<body onload="createMainMenu('${sessionScope.userLevel}', <%= Config.isMailQueueEnabled() %>, <%= Config.isSmsQueueEnabled() %>)">
	<div style="width: 1024px; margin: auto;">
		...
		<c:set var="LocalAddress" value="<%=Config.getRemoteServiceConfig().getLocalAddress()%>" />
	...
	<c:if test="${nmsType > 1}">
		<c:out value="<script type='text/javascript'>$(document).ready(getAllLocation('${LocalAddress}'));</script>" escapeXml="false"></c:out>
	</c:if>
</body>
</html>