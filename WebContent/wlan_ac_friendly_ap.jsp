...
<title>Friendly AP List ${param.ip}</title>
...
<body onload="createAcMenu('${param.ip}')">
	<div style="width: 1024px; margin: auto;">
		...
		<jsp:include page="/ACFriendlyRogueAP">
			<jsp:param value="FriendlyAPList" name="action" />
			<jsp:param value="${param.ip}" name="ip" />
		</jsp:include>

		<div id="page_content" align="center">
			<div id="page_title">Friendly AP List(${FriendlyAPList_size})</div>
			<%-- <div>
				<p align="center">
			...
			<table id="Log_Viewer_Table" class="Log_Viewer_Table">
				...
				<tbody>
					<c:forEach var="item" items="${FriendlyAPList}">
						<c:choose>
							<c:when test="${item[2] == 'none'}">
							</c:when>
							<c:when test="${item[2] != 'none'}">
								<tr class="a" onmouseover="this.style.backgroundColor='#ffffa2'" onMouseOut="this.style.backgroundColor=''">
